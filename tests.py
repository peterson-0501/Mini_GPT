...
def check_multiheadattention(model, checkpoint_file, device="cpu"):
    ...
    with torch.no_grad():
        print(f"Checkpoint key weight shape: {ckpt['model_state_dict']['transformer_layers.0.attention.head_0.key.weight'].shape}")
        print(f"Model head_0 key weight shape: {model.__getattr__(f'head_0').key.weight.shape}")
        ...
        print(f"Checkpoint key weight shape: {ckpt['model_state_dict']['transformer_layers.0.attention.head_1.key.weight'].shape}")
        print(f"Model head_1 key weight shape: {model.__getattr__(f'head_1').key.weight.shape}")
        ...
        print(f"Checkpoint key weight shape: {ckpt['model_state_dict']['transformer_layers.0.attention.head_2.key.weight'].shape}")
        print(f"Model head_2 key weight shape: {model.__getattr__(f'head_2').key.weight.shape}")
        ...
        print(f"Checkpoint key weight shape: {ckpt['model_state_dict']['transformer_layers.0.attention.head_3.key.weight'].shape}")
        print(f"Model head_3 key weight shape: {model.__getattr__(f'head_3').key.weight.shape}")
        ...
        print(f"Checkpoint out weight shape: {ckpt['model_state_dict']['transformer_layers.0.attention.out.weight'].shape}")
        print(f"Model out weight shape: {model.out.weight.shape}")
        ...
        print(f"Checkpoint out bias shape: {ckpt['model_state_dict']['transformer_layers.0.attention.out.bias'].shape}")
        print(f"Model out bias shape: {model.out.bias.shape}")
...
def check_miniGPT(model, checkpoint_file, device="cpu"):
    ...
    for key in state_dict.keys():
        if "transformer_layers" in key and "attention.head_" in key and ".causal_mask" in key:
            # Assuming each causal_mask tensor is of shape [512, 512]
